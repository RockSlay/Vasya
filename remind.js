    ...
    const args = message.content.split(' ').slice(1,2)
     ...
     if(args.length <= 0) {
       ...
       weatherbed.addField('Город', client.weather[message.guild.id].city== undefined ? 'Неизвестен':client.weather[message.guild.id].city)
        weatherbed.addField('Дата',(client.weather[message.guild.id].date == undefined ? 'Неизвестна': client.weather[message.guild.id].date.join("/")))

        
        message.channel.send(weatherbed)
        ...
        const args = message.content.split(' ').splice(2)
...
if(куча условий) {
 а дальше уже в программе напишу
}
}) */
